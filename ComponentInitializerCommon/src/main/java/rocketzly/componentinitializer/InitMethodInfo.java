...
public class InitMethodInfo implements Comparable<InitMethodInfo> {
    ...
    @Override
    public int hashCode() {
        return className == null ? -1 : className.hashCode();
    }

    @Override
    public boolean equals(Object obj) {
        if (!(obj instanceof InitMethodInfo)) {
            return false;
        }
        if (className == null) {
            return ((InitMethodInfo) obj).className == null;
        }
        return this.className.equals(((InitMethodInfo) obj).className);
    }
}
